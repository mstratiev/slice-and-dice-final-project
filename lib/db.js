    ...
    var addUser = (username, password) => {
        return new Promise((resolve, reject) => {
            readJSON((err, data) => {
                ...
                let d = data;
                d.users.push({
                    username:username, password:password, data: new Date(), id : userId+=1
                });
                writeJSON(d, (err) => {
                    if (!err) {
                        resolve(true)
    ...
    var addComment = (comment, user, postId) => {
        return new Promise((resolve, reject) => {
            readJSON((err, data) => {
                ...
                let currentUser = d.users.findIndex((u)=>{
                    return u.id = user.id})
                d.users[currentUser].comments.push(comment);
                d.posts[postId].comments.push(comment);
                writeJSON(d, (err) => {
                    if (!err) {
                        resolve(true)
                    }
